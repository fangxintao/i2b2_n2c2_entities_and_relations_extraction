...
class Selection_Dataset(Dataset):
    def __init__(self, hyper, dataset):
        ...
        for line in open(os.path.join(self.data_root, dataset), 'r'):
            ...
            # if int(instance['spo_list']['predicate']) == 0:
            #    self.spo_list0.append(instance['spo_list'])

    def __getitem__(self, index):
        selection = self.selection_list[index]
        text = self.text_list[index]
        # text = ' '.join(self.text_list[index])
        bio = self.bio_list[index]
        spo = self.spo_list[index]
        #spo0 = self.spo_list0[index]
        if self.hyper.cell_name == 'bert':
            ...
            #   charbert 与常规bert调用方式不同
            #     self.bert_tokenizer.basic_tokenizer.tokenize(text))
            # 这里需要输入的是字符？
    ...
    def pad_bert(self, text: List[str], bio: List[str], selection: List[Dict[str, int]]) -> Tuple[
        List[str], List[str], Dict[str, int]]:
        # for [CLS] and [SEP]
        text = ['[CLS]'] + text + ['[SEP]']
        bio = ['O'] + bio + ['O']
        selection = [{'subject': triplet['subject'] + 1, 'object': triplet['object'] +
                                                                   1, 'predicate': triplet['predicate']} for triplet in
                     selection]
...
class Batch_reader(object):
    def __init__(self, data):
        ...
        # tokens_id = indexer.as_padded_tensor(transposed_data[0])

        # self.tokens_id = pad_sequence(tokens_id,batch_first=True)
...
#   partial的作用是把某个函数的参数给固定住，并返回新的函数。
# conllate_fn用来将对应的数据合成一个batch的数据
# 内置函数__next__()会在循环里先自动调用返回一个batch
