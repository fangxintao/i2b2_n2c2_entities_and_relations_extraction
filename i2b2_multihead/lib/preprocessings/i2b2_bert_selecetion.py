...
class i2b2_bert_preprocessing(object):
    def __init__(self, hyper):
        ...
        self.bert_tokenizer = BertTokenizer.from_pretrained(
            'pretrained-models/general_bert')
    ...
    #           'bio': bio, 'selection': selection_dics}
        ...
        def align_bio(pos, bio):
            ...
            for i, p in enumerate(pos):
                if i != 0 and p == pos[i-1]:
                    # 1 != 0？？？？？
                    if bio[pos[i-1]] in ('B-treatment', 'B-test', 'B-problem'):
                        result.append('I' + '-' + bio[pos[i - 1]].split('-')[1])
                    else:
        ...
        def head2new(idx, aligned_tokens_pos):
            rev = list(reversed(aligned_tokens_pos)) # 返回一个反转的迭代器，之所以要反转，是为了得到子词最后一个成分的索引
        ...
        def spolist2new(spo_list, aligned_tokens_pos):
            # WARNING! strong side effect
            #  为什么说有很大的风险？直接对原本的词进行切词，每次的结果是否都是一致的？
            for triplet in spo_list:
                # triplet['subject'] = self.bert_tokenizer.basic_tokenizer.tokenize(' '.join(triplet['subject']))
                # triplet['object'] = self.bert_tokenizer.basic_tokenizer.tokenize(' '.join(triplet['object']))
        ...
        # bert_tokens = self.bert_tokenizer.basic_tokenizer.tokenize(' '.join(text))
    ...
    def _gen_one_data(self, dataset):
        ...
        with open(source, 'r') as s, open(target, 'w') as t:
            ...
            if len(sent) <= self.hyper.max_text_len:
                ...
                # (sent,selection_dics,bio)分别是代表 单词，(s,p,o),
                result = {'text': sent, 'spo_list': triplets,
                          'bio': bio, 'selection': selection_dics}
                result = self.prepare_bert(result)
                t.write(json.dumps(result))
    ...
    def gen_bio_vocab(self):
        ...
        result = {'<pad>': 7, 'B-problem': 0, 'I-problem': 1, 'B-test': 2, 'I-test': 3, 'B-treatment': 4, 'I-treatment': 5,
                  'O': 6}
        json.dump(result,
