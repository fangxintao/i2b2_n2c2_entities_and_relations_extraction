...
class MultiHeadSelection(nn.Module):
    def __init__(self, hyper) -> None:
        ...
        self.word_embeddings = nn.Embedding(num_embeddings=len(
            self.word_vocab), embedding_dim=hyper.emb_size)

        # initial word embedding with pre-trained word embeddings
        token_list = list(self.word_vocab.keys())
        if hyper.word_emb == 'glove':
            # Glove
            vec = text.vocab.GloVe(name='6B', dim=hyper.emb_size)
            weights = vec.get_vecs_by_tokens(token_list, lower_case_backup=True)
            self.word_embeddings = self.word_embeddings.from_pretrained(weights)

        elif hyper.word_emb == 'fasttext':
            # fastText
            vec = text.vocab.FastText()
            weights = vec.get_vecs_by_tokens(token_list, lower_case_backup=True)
            self.word_embeddings = self.word_embeddings.from_pretrained(weights)

        elif hyper.word_emb == 'word2vec':
            # biomedical word embeddings
            # download from http://bio.nlplab.org/
            vec = KeyedVectors.load_word2vec_format("word_embs/PubMed-and-PMC-w2v.bin", binary=True)
            token_vectors = []
            for token in token_list:
                if token in vec.wv.vocab.keys():
                    token_vectors.append(vec[token])
                else:
                    token_vectors.append([0]*hyper.emb_size)
            # convert list to FloatTensor
            weights = torch.cuda.FloatTensor(token_vectors)
            self.word_embeddings = self.word_embeddings.from_pretrained(weights)
        ...
        if hyper.cell_name == 'gru':
            ...
            self.encoder = BertModel.from_pretrained("bert_models/NCBI_BERT_pubmed_mimic_uncased_L-12_H-768_A-12/")
            # self.encoder = AutoModel.from_pretrained("bionlp/bluebert_pubmed_uncased_L-12_H-768_A-12")
        ...
        if self.hyper.cell_name == 'bert':
            ...
            #     'bert-base-uncased')
            self.bert_tokenizer = BertTokenizer.from_pretrained(
                'bert_models/NCBI_BERT_pubmed_mimic_uncased_L-12_H-768_A-12/')
            # self.bert_tokenizer = AutoTokenizer.from_pretrained("bionlp/bluebert_pubmed_uncased_L-12_H-768_A-12")
