...
class MultiHeadSelection(nn.Module):
    def __init__(self, hyper) -> None:
        ...
        # 词向量的输出维度是【seq_len,batch_size,embedding_size】
        ...
        if hyper.cell_name == 'gru':
            ...
            self.encoder = nn.LSTM(hyper.emb_size,
                                   hyper.hidden_size,
                                   bidirectional=True,
                                   batch_first=True)# batch_first如果为真，则输入和输出的Tensor为(batch,seq,feature)
            # a = nn.Sequential(nn.LSTM(),nn.LSTM(),nn.LSTM())
            ...
            self.encoder = BertModel.from_pretrained('pretrained-models/medical_bert')
            # self.encoder = CharacterBertModel.from_pretrained('pretrained-models/medical_character_bert')
            # self.encoder = BertModel.from_pretrained('pretrained-models/general_bert')
        ...
        self.selection_uv_dropout = nn.Dropout(0.1)

        self.emission = nn.Linear(hyper.hidden_size, len(self.bio_vocab) - 1)

        self.emission_drop = nn.Dropout(0.1)

        self.bert2hidden_drop = nn.Dropout(0.1)

        self.bert2hidden = nn.Linear(768, hyper.hidden_size)
        #self.bert2hidden = nn.Linear(1024, hyper.hidden_size)
        # for bert_lstm
        ...
        if self.hyper.cell_name == 'bert':
            ...
            self.bert_tokenizer = BertTokenizer.from_pretrained(
                'pretrained-models/medical_bert'
            )
            # self.bert_tokenizer = BertTokenizer.from_pretrained(
            #     'bert-base-uncased')
            # 通过词典导入分词器？
    ...
    def masked_BCEloss(self, mask, selection_logits, selection_gold):
        ...
        selection_loss = 5 * selection_loss
    ...
    @staticmethod
    def dev_description(epoch, epoch_num, output):
        return "L: {:.2f}, L_crf: {:.2f}, L_selection: {:.2f}, epoch: {}/{}:".format(
            output['dev_loss'].item(), output['dev_crf_loss'].item(),
            output['dev_selection_loss'].item(), epoch, epoch_num)
    ...
    def forward(self, sample, is_train: bool) -> Dict[str, torch.Tensor]:
        ...
        if self.hyper.cell_name in ('gru', 'lstm'):
            ...
            # bio_notpad = bio_gold != self.bio_vocab['<pad>']
            # mask = copy.deepcopy(text_list)
            # bio_mask = copy.deepcopy(text_list)
            # for i in range(len(bio_mask)):
            #     for j in range(len(bio_mask[i])):
            #         if text_list[i][j] == '[PAD]':
            #             bio_mask[i][j] = False
            #         elif text_list[i][j] == '[SEP]':
            #             bio_mask[i][j] = False
            #         else:
            #             bio_mask[i][j] = True
            ...
            # bio_mask = torch.tensor(bio_mask).cuda()  # fst token for crf cannot be masked
        ...
        dev_selection_loss = 0
        if not is_train:
            output['selection_triplets'] = self.inference(
                mask, text_list, decoded_tag, selection_logits)
            output['spo_gold'] = spo_gold
            dev_selection_loss = self.masked_BCEloss(mask, selection_logits,
                                                 selection_gold)

            output['dev_selection_loss'] = dev_selection_loss
            dev_loss = dev_crf_loss + dev_selection_loss
            output['dev_loss'] = dev_loss
            output['dev_description'] = partial(self.dev_description, output=output)
    ...
    def selection_decode(self, text_list, sequence_tags,
        ...
        def find_entity(pos, text, sequence_tags):
            ...
            if sequence_tags[pos] in ('B-problem', 'B-treatment', 'B-test', 'O'):
